...
SoundSource::SoundSource()
{
	...
	Engine().GetSoundEngine().AddSoundSource(this);
}
...
void SoundSource::Init(wchar_t* filePath)
{
	m_isAvailable = false;
	//検索
	m_waveFile = Engine().GetSoundEngine().GetWaveFileBank().FindWaveFile(0, filePath);
	if (!m_waveFile) {
		...
		if (result == false) {
			//waveファイルの読み込みに失敗。
			//登録解除
			Engine().GetSoundEngine().GetWaveFileBank().UnregistWaveFile(0, m_waveFile);
			m_waveFile.reset();
			return;
		}
		m_waveFile->AllocReadBuffer(m_waveFile->GetSize());	//waveファイルのサイズ分の読み込みバッファを確保する。
		//登録
		Engine().GetSoundEngine().GetWaveFileBank().RegistWaveFile(0, m_waveFile);
		unsigned int dummy;
	...
	//サウンドボイスソースを作成。
	m_sourceVoice = Engine().GetSoundEngine().CreateXAudio2SourceVoice(m_waveFile.get(), false);
...
void SoundSource::InitStreaming(wchar_t* filePath, unsigned int ringBufferSize, unsigned int bufferSize)
{
	m_isAvailable = false;
...
void SoundSource::Release()
{
	...
	//サウンドソースの削除
	Engine().GetSoundEngine().RemoveSoundSource(this);
}
